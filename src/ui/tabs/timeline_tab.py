...
class TimelineTab(QWidget):
    ...
    def __init__(self, parent=None, logger: Optional[logging.Logger] = None):
        ...
        self.use_utc = True  # Default to UTC time
    ...
    def _create_timeline_table(self) -> QWidget:
        ...
        self.timeline_table.setColumnCount(8)
        self.timeline_table.setHorizontalHeaderLabels([
            'Time', 'Event Type', 'Level', 'Activity', 'User', 'Program', 'Path', 'Details'
        ])
    ...
    def _create_detail_panel(self) -> QWidget:
        """Create event detail panel widget."""
    ...
    def load_events(self, events_df: pd.DataFrame) -> None:
        ...
        Args:
            events_df: Pandas DataFrame with normalized and classified events
        """
    ...
    def _populate_table(self) -> None:
        """Populate timeline table with filtered events."""
        ...
        for row_idx, (_, event) in enumerate(self.filtered_df.iterrows()):
            # Timestamp
            ts = event['ts_utc'] if self.use_utc else event['ts_local']
            ts_item = QTableWidgetItem(str(ts))
            self.timeline_table.setItem(row_idx, 0, ts_item)
            
            # Classification
            rule_class = event.get('rule_class', 'UNKNOWN')
            class_item = QTableWidgetItem(rule_class)
            self.timeline_table.setItem(row_idx, 1, class_item)
            
            # Severity
            severity = event.get('severity', 0)
            severity_item = QTableWidgetItem(str(severity))
            self.timeline_table.setItem(row_idx, 2, severity_item)
            
            # Event Type
            event_type = event.get('event_type', '')
            type_item = QTableWidgetItem(event_type)
            self.timeline_table.setItem(row_idx, 3, type_item)
            
            # User
            user = event.get('user_account', '')
            user_item = QTableWidgetItem(user)
            self.timeline_table.setItem(row_idx, 4, user_item)
            
            # Executable
            exe = event.get('exe_name', '')
            exe_item = QTableWidgetItem(exe)
            self.timeline_table.setItem(row_idx, 5, exe_item)
            
            # Path
            path = event.get('filepath', event.get('artifact_path', ''))
            path_item = QTableWidgetItem(path)
            self.timeline_table.setItem(row_idx, 6, path_item)
            
            # Description
            desc = event.get('description', '')
            desc_item = QTableWidgetItem(desc)
            self.timeline_table.setItem(row_idx, 7, desc_item)
            
            # Color-code row by classification
        ...
        # Update count label
        self.event_count_label.setText(f"Events: {len(self.filtered_df)}")
